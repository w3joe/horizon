# Linux shared assurance-control lane scheduling
...
numbered CPU to a shared assurance-control lane containing the gate and assurance
processes. Fusion, simulator, decision AI, collector, console, and optional perception
are confined to the remaining support-lane CPUs. This keeps both predictive control
validators away from high-rate plant, ingestion, AI, and UI service work. Every child
process retains its inherited default scheduling priority. The system-test orchestrator
...
affinity keeps support-lane Horizon services off the assurance-control CPU, but gate
and assurance still share it, and the policy cannot exclude kernel work,
