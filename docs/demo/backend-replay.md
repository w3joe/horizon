...
.venv/bin/python scripts/demo_capture.py \
  --output ../horizon-runs/demo/unsafe-route-v2
```
...
- `GET /api/demo/runs/unsafe-route-v2` returns `{manifest, ...replay}` after checking the directory ID, schemas, clean-source marker, size cap, and replay SHA-256.
