...
**Status:** Proposed for user review; execution has not started.  
...
3. [Dataset and repository survey](output/runtime-assurance/maritime_data_survey.md): acquisition options, licensing limitations, and data gaps.
4. [Existing sample provenance](output/runtime-assurance/data-samples/provenance.json): two small inspected starter files.
...
**Review checkpoint:** This Markdown file is the current deliverable. Once its scope and schedule are accepted, launch **A01 first**, then proceed through the concurrent work packets, including the scheduled later realism phase, without requesting repetitive permission for ordinary implementation steps already covered by that authorization.
