    ...
    <section className="timeline" aria-label="Scenario replay timeline">
      ...
      <div className="timeline-track">
        ...
        <div className="event-markers">
          {props.events.map((event) => <button key={event.id} type="button" className={`${event.severity} ${props.selectedEventId === event.id ? "selected" : ""}`} style={{ left: `${(event.timeS / props.durationS) * 100}%` }} onClick={() => props.onSelectEvent(event.id)} aria-label={`${event.stage}: ${event.label} at ${event.timeS} seconds`}><i /><span>{event.stage}</span></button>)}
        </div>
      </div>
